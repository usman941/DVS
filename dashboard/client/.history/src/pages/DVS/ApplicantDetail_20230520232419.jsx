...
const HandleDownload = async () => {
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_BACKEND_URL}/download_docs/${Location.state.degree_reg_no}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('AuthToken')}`,
        },
    ...
    if (response.status === 200) {
      ...
      const filename = filenameMatch ? filenameMatch[1] : 'file.zip'; // Default filename if not found
