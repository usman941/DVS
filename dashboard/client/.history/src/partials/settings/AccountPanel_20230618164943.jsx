...
function AccountPanel() {
  ...
  // useEffect(()=>{
  //   setProfilePic(profile?.data?.user?.image)
  //   console.log(profile?.data?.user?.image);
  // },[profile?.data]);
  ...
  const handleUpload = async () => {
    try {
      ...
      const file = await fetch(profilePic).then((response) => response.blob());
  ...
  return (
    <div className="grow">
      ...
      <div className="flex items-center">
        <div className="mr-4">
          <img className="w-20 h-20 rounded-full" src={profile.image} width="80" height="80" alt="User upload" />
        </div>
