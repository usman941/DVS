...
const Quries = () => {
  ...
  const sendFeedback=async()=>{
    try {
      const result=await axios.put(`${import.meta.env.VITE_BACKEND_URL}/adminResponsebymail`,
      {
        queryId:Itemdata.id,
   ...
   
      {
        ...
        renderCell: (params) => (
          ...
          <FcFeedback
            ...
            //   OpenDelete(params.row);
            // }}
            onClick={(e) => { e.stopPropagation(); setFeedbackModalOpen(true); setItemData(params.row);  }}
  ...
  const DeleteTask = () => {
    ...
    try {
      axios
        .delete(`${import.meta.env.VITE_BACKEND_URL}/deleteQueries/${Itemdata.id}`, 
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("AuthToken")}`,
          },
        ...
        .then((response) => {
          ...
          setOpen(false);
          setApplicants((data) => data.filter((item) => item.id !== Itemdata?.id));
        })
        .catch((err) => {
          console.log(err);
