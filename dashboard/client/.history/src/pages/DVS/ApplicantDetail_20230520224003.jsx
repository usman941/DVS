...
const HandleDownload=async()=>{
  try {
    const result=await axios.get(`${import.meta.env.VITE_BACKEND_URL}/download_docs`,
    {
      degree_reg_no:Location.state.degree_reg_no,
    },{
      headers:{
        Authorization:`Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2NDY3NjA2NDNkM2E2ZDcwNWVhMDhjNzYiLCJpc0FkbWluIjp0cnVlLCJpYXQiOjE2ODQ2MDQxMzd9.0Utk1WdKPuklXPVD-Cohc5LeiRlfjOowkfBrm90lmPQ`
      }
    }
    )
    console.log("download pdf  result:",result);
     toast.success(result.data.msg)
   } catch (error) {
    console.log("Error While download  the  document",error);
   }
}
  return (
